...
// var appConfig = require(path.resolve(process.cwd(), './divshot.json')); // TODO: make this dynamic
var appConfig = {};
...
// cli.flag('-w', '--with')
//   .description('script names to run in parellel with the server')
//   .handler(function () {
//     var scripts = parseScripts(arguments[0].split(','));
//     cli.scripts['with'] = cli.scripts['with'].concat(scripts);
//   });
...
function parseScripts (scripts) {
  return _.filter(scripts, function (script) {
    return Object.keys(appConfig.scripts).indexOf(script) > -1;
  });
}
