...
function redirectAsCleanUrl(req, res) {
  ...
  var redirectUrl = (pathutils.isDirectoryIndex(pathname))
...
module.exports = function(spec, config) {
  var rules = config.cleanUrls || false;
  ...
  return function(req, res, next) {
    ...
    if (pathutils.isDirectoryIndex(pathname) && pathMatchesRules(pathname, rules)) {
      return redirectAsCleanUrl(req, res);
    }
