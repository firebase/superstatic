...
module.exports = function (imports) {
  ...
  function send (data) {
    ...
    if (len !== undefined && (isHead || req.method === 'GET')) {
      if (shouldEtag && !res.getHeader('ETag') && provider.generateEtag) {
        etag = provider.generateEtag(data, 'utf8');
      }
    }
    ...
    onHeaders(res, function () {
      ...
      if (provider.cacheIsFresh && provider.cacheIsFresh(req, res)) {
        res.statusCode = 304;
  ...
  function sendFile (pathname) {
    ...
    provider.exists(pathname, function (exists) {
      ...
      if (!exists) {
        ...
        var len = provider.statSync(pathname).size;
        ...
        if (len !== undefined && (isHead || req.method === 'GET')) {
          if (shouldEtag && !res.getHeader('ETag') && provider.generateEtag) {
            etag = provider.generateEtag(provider.statSync(pathname), 'utf8');
          }
        }
