...
var reqres = require('./utils/reqres');
...
module.exports = function (imports) {
  ...
  function send (data) {
    ...
    // TODO: move etag generation to provider(s)
    // ETag support
    var shouldEtag = true;
    var isHead = req.method === 'HEAD';
    var etag;
    if (len !== undefined && (isHead || req.method === 'GET')) {
      if (shouldEtag && !res.getHeader('ETag')) {
        etag = reqres.generateEtag(data, 'utf8');
      }
    }
    ...
    onHeaders(res, function () {
      ...
      if (reqres.fresh(req.headers['if-none-match'], res.getHeader('ETag'))) {
        res.statusCode = 304;
