...
module.exports = function (imports) {
  ...
  function sendFile (pathname, existingStream) {
    var stream = existingStream || through();

    provider.get(pathname, function(err, meta, readStream) {
      if (err && !existingStream) {
        return sendFile(path.join(pathname, imports.config.index || INDEX_FILE), stream);
      } else if (err) {
        // res.statusCode = 404;
        // res.end('Not Found');
