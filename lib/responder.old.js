...
module.exports = function (imports) {
  ...
  function sendFile (pathnames, existingStream, previousPath) {
    var stream = existingStream || through();

    if (_.isString(pathnames)) {
      var indexPath = path.join(pathnames, indexFile);
      if (_.endsWith(pathnames, '/') && !existingStream) {
        return sendFile([indexPath], stream, pathnames);
      } else {
        pathnames = [pathnames, indexPath];
      }
    }

    var currentPath = pathnames.shift();
    provider.get(req, currentPath, function(err, meta, readStream) {
      if (err && pathnames.length) {
        return sendFile(pathnames, stream, currentPath);
      } else if (err) {
        ...
        previousPath &&
        _.endsWith(currentPath, indexFile) &&
      ...
      var contentType = mime.contentType(path.extname(currentPath));
      onHeaders(res, function() {
        res.setHeader('Content-Type', contentType);
