...
ConfigRedis.prototype.getRelease = function (hostname, callback) {
  ...
  async.parallel({
    build: function (cb) {
      self._getBuildCacheKey(hostname, function (err, key) {
        self._getBuildCache(key, cb, hostname);
      });
    },
    env: function (cb) {
      self._getEnvCacheKey(hostname, function (err, key) {
...
ConfigRedis.prototype._getBuildCacheKey = function (hostname, callback) {
  var self = this;
  
  this._get(self._hostCacheKey(hostname), function (err, key) {
    if (err) return callback(err);
    if (key) return callback(null, key);
    
    self._getReleaseFromApi(hostname, function (err, release) {
      self._set(self._hostCacheKey(hostname), self._buildCacheKey(release), function (err) {
        self._getBuildCacheKey(hostname, callback);
      });
    });
  });
};
...
ConfigRedis.prototype._getEnvCacheKey = function (hostname, callback) {
  var self = this;
  
  this._get(self._envCacheKey(hostname), function (err, key) {
    if (err) return callback(err);
    if (key) return callback(null, key);
    
    self._getReleaseFromApi(hostname, function (err, release) {
      self._set(self._envCacheKey(hostname), self._appEnvCacheKey(release), function (err) {
        self._getEnvCacheKey(hostname, callback);
      });
    });
  });
};
...
ConfigRedis.prototype._set = function (key, value, callback) {
  ...
  if (parsed.err) return callback(parsed.err);
  this.client.set(key, parsed.string, callback);
};
