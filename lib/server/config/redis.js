...
ConfigRedis.prototype.load = function (hostname, callback) {
  ...
  this.getRelease(hostname, function (err, release) {
    ...
    self.build = self._buildReleaseObject(release, hostname);
    callback(err, self.build);
  });
};

ConfigRedis.prototype._buildReleaseObject = function (rawReleaseObject, hostname) {
  var release = Object.create(rawReleaseObject);
