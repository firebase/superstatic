...
SERVICES.redirects = function (req, res, next) {
  var globject = require('globject');
  var slash = require('slasher');
  var redirects = globject(slash(req.config.services.redirects));
...
SERVICES.redirects.matchesRequest = function (req, done) {
  var slash = require('slasher');
  var redirects = slash(req.config.services.redirects || {});
  
  done(redirects.hasOwnProperty(req.url));
};
var SERVICE_PREFIX_ROUTE = '__';
//////////////////////////////////////////////////////
...
var Server = function (options) {
  ...
  this._middlewareChain = [
    ...
    //
    middleware.services(SERVICES, SERVICE_PREFIX_ROUTE),
