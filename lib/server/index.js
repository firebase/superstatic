...
SuperstaticServer.prototype._createServer = function () {
  ...
  return this._server = connect()
    ...
    // Setup 
    .use(middleware.configure(this))
    .use(middleware.removeTrailingSlash())
    .use(middleware.restful())
    
    // Serve files
    .use(middleware.protect(settings))
    .use(middleware.sender())
    .use(middleware.cacheControl())
    .use(middleware.customRoute(settings, store))
    .use(middleware.cleanUrls(settings, store))
    .use(middleware.static(settings, store))
