...
// TODO: Should be in own module
middleware.stack = function(routes, settings, store) {
  var defaultStack = connect()
  ...
  return function(req, res, next) {
    // TODO: swap stack based on value of `req.config.stack`
    defaultStack(req, res, next);
  }
};
...
SuperstaticServer.prototype._createServer = function () {
  ...
  return this._server = connect()
    // Standard request logging/compression
    .use(connect.logger())
    .use(connect.query())
    .use(connect.compress())
  
    // Universal Setup (stack switching makes no sense if config isn't loaded)
    .use(middleware.configure(settings))
    .use(middleware.removeTrailingSlash())
  
    // Hand off specific handling to the stack
    .use(middleware.stack(routes, settings, store))
  
    // Universal favicon/error handling.
