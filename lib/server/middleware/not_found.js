...
var notFound = function (req, res, next) {
  if (!req.superstatic || !req.superstatic.path) {
    return internals.serveFile(res, getErrorPagePath(req), req.ss.store);
  }
...
function hasErrorPagePath (req) {
  return !!(req.ss.config && req.ss.config.error_page && fs.existsSync(req.ss.config.error_page));
}
