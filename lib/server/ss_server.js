...
Superstatic.prototype._createServer = function () {
  return this._server = connect()
    ...
    .use(this.router.forceTrailingSlash.bind(this.router))
...
Superstatic.prototype.start = function (callback) {
  this._createServer();
  this._openServer = this._server.listen(this._port, callback);
};

Superstatic.prototype.stop = function (callback) {
  var self = this;
  this._openServer.close(function () {
    this._server = null;
    process.nextTick(function () {
      callback();
    });
  });
  // TODO: figure out how to stop the server
  
  // this._server.stop({timeout: 100}, function (err) {
  //   delete self._server;
  //   callback(err);
  // });
};
