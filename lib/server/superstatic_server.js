...
SuperstaticServer.prototype._createServer = function () {
  ...
  /*
    ...
    ---------------
      1. x Custom routes
      2. x Clean urls
      3. x Static (includes directory index)
      4. * Favicon
   */
  return this._server = connect()
    ...
    // Clean urls
    .use(function (req, res, next) {
      if (!req.ss.config.clean_urls) return next();
    ...
    // Static
    .use(function (req, res, next) {
      if (!req.ss.settings.isFile(fullPath(req, req.ss.pathname))) return next();
      
      send(req, req.ss.pathname)
        .root(req.ss.config.cwd)
        .on('error', function (err) {
          next()
        })
