var _ = require('lodash');
var Router = require('router');
var send = require('send');
var basicAuth = require('basic-auth-connect');
var redirects = require('redirects');
var setHeaders = require('set-headers');
var cacheControl = require('cache-control');
var favicon = require('serve-favicon');

var dfs = require('./dfs');
var responder = require('./responder');
var middleware = require('./middleware');
var loadConfigFile = require('./loaders/config-file');
var loadEnvFile = require('./loaders/env-file');

var FAVICON_PATH = __dirname + '/assets/favicon.ico';
var DEFAULT_ERROR_PAGE = __dirname + '/assets/not_found.html';
var INDEX_FILE = 'index.html';
var CWD = process.cwd();

module.exports = function (spec) {
  
  spec = spec || {};
  var router = Router();
  var cwd = spec.cwd || CWD;
  
  // Load data
  var config = loadConfigFile(spec.config);
  var env = loadEnvFile(spec.env);
  
  // Set up provider
  var provider = spec.provider || dfs(_.extend({
    index: INDEX_FILE, // default index file
    cwd: cwd // default current working directory
  }, config));
  var defaultErrorPage = spec.errorPage || DEFAULT_ERROR_PAGE;
  
  // Setup helpers
  router.use(function (req, res, next) {
    
    responder({
      res: res,
      provider: provider
    });
    
    next();
  });
  
  // Protect
  if (spec.protect) {
    router.use(basicAuth.apply(basicAuth, spec.protect.split(':')));
  }
  
  // Services - TODO: implement
  // if (spec.services) {
  //   router.use(services(config));
  // }
  
  // Only set up stack if there is a config
  if (!_.isEmpty(config)) {
    
    // Redirects
    if (config.redirects) {
      router.use(redirects(config.redirects));
    }
    
    // Headers
    if (config.headers) {
      router.use(setHeaders(config.headers));
    }
    
    // Remove trailing slash
    router.use(middleware.slashify({
      config: config,
      provider: provider
    }));
    
    // Cache control
    if (config.cache_control) {
      router.use(cacheControl(config.cache_control));
    }
    
    // Clean urls
    if (config.clean_urls) {
      router.use(middleware.cleanUrls({
        rules: config.clean_urls,
        config: config,
        provider: provider
      }));
    }
    
    // Environment variables
    middleware.env({
      data: env,
      router: router
    });
    
    // Static router
    router.use(middleware.staticRouter({
      routes: config.routes
    }));
    
    // Static files
    router.use(middleware.static());
  }
  // end test for empty config
  
  // default favicon
  router.use(favicon(FAVICON_PATH));
  
  // Custom not found
  if (config.error_page) {
    router.use(middleware.customNotFound({
      config: config
    }));
  }
  
  // TODO: these should only be used in the server file.
  // No need to have these when Superstatic is middleware
  // Default not found
  router.use(middleware.notFound({
    file: defaultErrorPage
  }));
  
  // Ensure the server never crashes
  // Error handler
  router.use(middleware.finalHandler({
    file: DEFAULT_ERROR_PAGE
  }));
  
  return router;
};