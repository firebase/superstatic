...
function initializeCompressor(options) {
  var compressor;
  try {
    compressor = require('shrink-ray')(options);
  } catch (e) {
    compressor = require('compression')(options);
  }
  return compressor;
}
...
var superstatic = function(spec) {
  ...
  // Select usable compression library
  if (spec.gzip && spec.compression === undefined) {
    spec.compression = true;
  }
  var compressionConfig = (_.isObject(spec.compression) ? spec.compression : {});
  var compressor = initializeCompressor(compressionConfig);
